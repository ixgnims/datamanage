...
export default {
  ...
  data() {
    ...
    const materialTableOptions = {
      ...
      columns: [
        ...
        {field: 'total_estimated_profit', title: '预估利润', width: 100, showHeaderOverflow: true, formatter: formatFloat, sortable: true},
        {field: 'video_level', title: '视频级别', width: 100, showHeaderOverflow: true},
      ],
      data: [],
      footerData: [],
      footerRowStyle () {
    ...
    return {
      ...
      materialChartOptions: {
        ...
        yAxis: [
          {type: 'value', name: this.activeLabel, splitLine: {show: false}}
        ],
        dataZoom: [],
        series: [],
        grid: {
  ...
  methods: {
    ...
    setMaterialAlert() {
      this.alertList = []
      if (this.tag_type === "LIVING" && this.parentMaterialAlert.length > 0) {
        for (let i = 0; i < this.parentMaterialAlert.length; i++) {
          let title_str = this.parentMaterialAlert[i]['material_id']+" "+this.parentMaterialAlert[i]['file_title']
          let description_str = " " + "过去5分钟低消耗"
          if (this.parentMaterialAlert[i]['related_ad_list'].length > 0) {
            description_str += " " + "计划id（" + this.parentMaterialAlert[i]['related_ad_list'] + "）"
          }
          this.alertList.push({
            title: title_str,
