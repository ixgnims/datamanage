        ...
        <el-card shadow="hover" style="margin-top: 5px">
          <el-tabs v-model="activeTab" @tab-click="handleTabClick">
            <el-tab-pane label="单量ROI" name="Order2">
              <Shuangyu49_orderChart2 :activeTab="activeTab" :selectLive="selectLive" :last_order_cnt="last_order_cnt" :roiInput="roiInput" :order_goal="order_goal"/>
            </el-tab-pane>
            <el-tab-pane label="投流视频" name="Video">
              <Shuangyu49_materialChart @change-selectMetric="selectMetricChange" @change-filterDimension="filterDimensionChange"
...
import Shuangyu49_materialChart from "./Shuangyu49_materialChart.vue";
import Shuangyu49_orderChart2 from "./Shuangyu49_orderChart2.vue";
export default {
  name: "Shuangyu49_Estimate",
  components: {
    Shuangyu49_orderChart2,
    Shuangyu49_materialChart,
  },
  data() {
  ...
  methods: {
    ...
    getLiveList() {
      ...
      axios.get(`http://47.107.244.209:8967/dashboard/shuangyu/dy/live/room/list/get/?query_start=${start_date}&query_end=${end_date}`)
          .then(response => {
    ...
    getLiveData() {
      if (this.selectLive) {
        ...
        // 表格数据接口
        axios.get(`http://47.107.244.209:8967/dashboard/shuangyu/dy/live/room/estimate/get/?room_id=${room_id}&roi_goal=${roi_goal}&live_date=${live_date}&data_source=${data_source}`)
            .then(response => {
    ...
    getLiveTrend(room_id) {
      axios.get(`http://47.107.244.209:8967/dashboard/shuangyu/dy/trend/get/?room_id=${room_id}`)
          .then(response => {
    ...
    getMaterialTrend(start_time, end_time, advertiser_list) {
      // 折线图
      axios.get(`http://47.107.244.209:8967/dashboard/shuangyu49/dy/live/material/trend/get/?start_time=${start_time}&advertiser_list=${advertiser_list}&end_time=${end_time}&room_id=${this.selectLive}&dimension=${this.filterDimension}&select_metric=${this.selectMetric}&sort_metric=${this.sortMetric}&search_type=${this.searchType}&query_item=${this.queryItem}&record_limit=${this.recordLimit}`)
          .then(response => {
            if (response.data.code === 1) {
      ...
      axios.get(`http://47.107.244.209:8967/dashboard/shuangyu49/dy/live/material/table/get/?live_date=${live_date}&room_id=${this.selectLive}&advertiser_list=${advertiser_list}&dimensions=material_id&having_info=True`)
          .then(response => {
            if (response.data.code === 1) {
              this.parentMaterialPivot = response.data.pivot_data
              console.log(this.parentMaterialPivot)
              this.footerSum = response.data.footer_sum
              console.log(this.footerSum)
