  ...
  <div>
    <el-row style="text-align: right;">
      ...
      <el-col :span="6" style="text-align:right" >
        <el-button circle type="text" @click="exportDataEvent" icon="el-icon-download"></el-button>
        <el-switch
            v-model="value"
            active-color="#13ce66"
            active-text="自动更新"
    ...
    <el-row>
      ...
      <vxe-table
        ...
        <vxe-column
            field="ALiu_indirect_order_amount_7days_thisMonth"
            title="7日支付"
            :formatter="rounding"
            sort-type="number"
            sortable>
...
export default {
  ...
  data () {
    return {
      update_time: '',
      tableData: [],
      defaultExpandKeys: ["1", "3"],
      footerData: [
        {
          account_name: '合计', cost_diff: '0', click_diff: '0', stat_cost: '0', //delivery_cnt: '0',
          pay_order_amount: '0', indirect_order_pay_gmv_7days: '0', ALiu_indirect_order_amount_7days_thisMonth: '0',
  ...
  methods: {
    async getData() {
      try {
        // 尝试API直连
        const response = await axios.get(`/marketing/today/table/test/`);
        this.tableData = response.data;
        this.update_time = response.data[0].input_time;
        let total = response.data.filter(item => item.parentId===null)
        var cost_diff_sum = total.map(item => item.cost_diff).reduce((a, b) => a + b);
        cost_diff_sum = parseFloat(cost_diff_sum.toFixed(2));
        var click_diff_sum = total.map(item => item.click_diff).reduce((a, b) => a + b);
        var stat_cost_sum = total.map(item => item.stat_cost).reduce((a, b) => a + b);
        stat_cost_sum = parseFloat(stat_cost_sum.toFixed(2));
        // var delivery_cnt_sum = total.map(item => item.delivery_cnt).reduce((a, b) => a + b);
        var pay_order_amount_sum = total.map(item => item.pay_order_amount).reduce((a, b) => a + b);
        var indirect_order_pay_gmv_7days = total.map(item => item.indirect_order_pay_gmv_7days).reduce((a, b) => a + b);
        var ALiu_indirect_order_amount_7days_thisMonth = total.map(item => item.ALiu_indirect_order_amount_7days_thisMonth).reduce((a, b) => a + b);
        ALiu_indirect_order_amount_7days_thisMonth = parseFloat(ALiu_indirect_order_amount_7days_thisMonth.toFixed(2));
        if (stat_cost_sum > 0) {
        ...
        this.footerData = [{
          ...
          ALiu_indirect_order_amount_7days_thisMonth: ALiu_indirect_order_amount_7days_thisMonth,
