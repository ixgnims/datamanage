...
export default {
  ...
  methods: {
    async getData() {
      try {
        // 尝试API直连
        const response = await axios.get(`/marketing/today/table/aliu/`);
        ...
        var ALiu_indirect_order_amount_7days_thisMonth = response.data.map(item => item.ALiu_indirect_order_amount_7days_thisMonth).reduce((a, b) => a + b);
